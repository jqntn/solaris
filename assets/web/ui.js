(() => {
  ...
  const hasKeyboardFocusVisible = () => keyboardFocusVisible;
  ...
  const moveDirectionalFocus = (event, items, options) => {
  ...
  const installButtonConfirmKeys = ({ root = document, activate = (button) => button.click() } = {}) => {
  ...
  const createModal = ({ modal, defaultFocus = null }) => {
  ...
  const createConfirmDialog = ({ modal: modalEl, cancelButton, confirmButton, onConfirm = () => {} }) => {
